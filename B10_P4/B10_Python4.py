...
if fsb==True:
    with st.expander("Kế hoạch đã chọn"):
        st.write(f"Bạn đã chọn món khai vị:")
        if len(chonmkv)==0:
            st.warning("Bạn chưa chọn món khai vị")
        else:
            for monan in chonmkv:
                st.write(monan)
        st.write(f"Bạn đã chọn món chính:")
        st.write(chonmc)
        st.write(f"Bạn đã chọn món tráng miệng:")
        if len(chonmtm)==0:
            st.warning("Bạn chưa chọn món tráng miệng")
        else:
            for monan in chonmtm:
                st.write(monan)
        st.write(f"Bạn đã chọn đồ uống:")
        st.write(chondu)
        st.write(f"Bạn đã chọn lịch trình: Từ {chonlt[0]} đến {chonlt[1]}")